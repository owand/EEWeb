...
var lnf = 'По Вашему запросу не найдено ни одного документа.<br><br>Удостоверьтесь, что все ключевые слова записаны правильно.<br>Попробуйте другие или более общие ключевые слова.';
var lp = 'Предыдущие ';
var ln = 'Следующие ';
...
function tip_out() {
	...
	for (var a = tn; a < nd; a++) {
		...
		if (os[5] == '1') {
			document.write('<a href="', os[2], '" target="main">', os[1], '</a>');
		}
		if (os[5] != '0' && os[5] != '1') {
		...
		if (include_url == 1) {
			if (os[5] == '0') {
				document.write('<br><a href="', os[2], '">', os[2], '</a><br><br>');
			}			
			if (os[5] == '1') {
				document.write('<br><a href="', os[2], '" target="main">', os[2], '</a><br><br>');
			}
			if (os[5] != '0' && os[5] != '1') {
				document.write('<br><a href="', os[2], '" target="', os[5], '">', os[2], '</a><br><br>');
			}
		} else {
			document.write('<p>');
		}
